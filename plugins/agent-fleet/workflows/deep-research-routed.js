...
// PROVENANCE (agent-fleet plugin): this file is the built-in deep-research
// workflow script as persisted by Claude Code 2.1.223 (base sha256
...
const searchResults = await pipeline(
  ...
  angle => agent(SEARCH_PROMPT(angle), {
    ...
    // Long tool loop, closed schema: the cheapest tier holds breadth (bench/
    // batch 2026-08-07-breadth-tier: no fidelity difference vs mid tier).
    model: "haiku"
  }).then(r => {
    if (!r) return null
    ...
    return parallel(
      novel.map(source => () => {
        ...
        return agent(FETCH_PROMPT(source, searchResult.angle), {
          ...
          // Same rule as search: fetch-and-extract is a long tool loop with a
          // closed schema; quotes are string-checked downstream.
          model: "haiku",
        }).then(ext => {
    ...
    parallel(
      Array.from({ length: VOTES_PER_CLAIM }, (_, v) => () =>
        agent(VERIFY_PROMPT(claim, v), {
          ...
          // The adversarial pass is where depth pays, and paired runs put the
          // mid tier even with the top model at contesting (18/20 vs 17/20).
          // This is also the bulk of the fleet: up to 25 claims x 3 votes.
          model: "sonnet",
        })
      )
    ).then(verdicts => {
...
// Synthesis inherits the session model on purpose: it is the one stage the
// agent-fleet rules would rather hand to the orchestrator itself.
