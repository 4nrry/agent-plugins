...
| [media-agent](plugins/media-agent/) | Producao de midia numa maquina propria com GPU de 8 GB: locucao a -16 LUFS com `say` separado da legenda, trilha e bancos com a licenca de cada item, retoque no ComfyUI so com modelos comerciais, turntable no Blender headless, video como codigo e edicao de video pronto pelo MCP do Kinocut. Um hook que avisa onde o `vipsthumbnail -o` vai gravar de verdade. |
